...
export class AuthGuard {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
    ...
    if (currentUser) {
      if (state.url.includes('memories/new')) {
        if (this.authService.getAuthFromLocalStorage()?.isPaymentRequired) {
          this.router.navigate(["/payment"], {
            queryParams: {
              typeId: 1,
              role: currentUser.roles.includes("2") ? 2 : currentUser.roles.includes("3") ? 3 : 4,
            }
          });
          return false;
        }
      ...
      // else if (state.url.includes('memories/')) {
      //   var memoryId = parseInt(state.url.split('/')[state.url.split('/').length - 1]);

      //   if(memoryId) {
      //     this.memoryManagementService.getById(memoryId).subscribe(result => {
      //       if (result.isSuccess) {
      //         if ((result.data.isLinkOnly || result.data.isPrivate) && result.data.userId != currentUser?.id) {
      //           this.router.navigate(['/memories']);
      //           return false;
      //         }
      //         else {
